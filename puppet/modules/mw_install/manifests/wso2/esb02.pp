class mw_install::wso2::esb02 {
    ...
    $smtp_from = "wso2esb02@wso2-all-in-one"

    ##### Configurable parameters ends here #####

    mw_install::filecopy { "wso2esb-4.9.0_wso2esb02":
        file          => "wso2esb-4.9.0",
        origin        => "/opt/wso2",
        destination   => "/opt/wso2/wso2esb02",
        user          => "root",
        group         => "root",
    ...
    file { "/opt/wso2/wso2esb02/repository/components/lib/mysql-connector-java-5.1.38-bin.jar":
        mode    => 644,
        owner   => "root",
        group   => "root",
        source  => '/vagrant/_downloads/mysql-connector-java-5.1.38-bin.jar',
        #require => File["/opt/wso2/wso2esb02"],
    }  -> 

    # WSO2ESB startup script
    file { '/opt/wso2/wso2esb02/bin/wso2server.sh':
        mode    => 755,
        owner   => "root",
        group   => "root",
        content => template('mw_install/wso2esb02/bin/wso2server.sh.erb'),
        #require => File["/opt/wso2/wso2esb02"],
    }  -> 

    # Template for the master datasource configuration
    file { '/opt/wso2/wso2esb02/repository/conf/datasources/master-datasources.xml':
        mode    => 644,
        owner   => "root",
        group   => "root",
        content => template('mw_install/wso2esb02/repository/conf/datasources/master-datasources.xml.erb'),
        #require => File["/opt/wso2/wso2esb02"],
    }  -> 

    # Template for conf/user-mgt.xml
    file { '/opt/wso2/wso2esb02/repository/conf/user-mgt.xml':
        mode    => 644,
        owner   => "root",
        group   => "root",
        content => template('mw_install/wso2esb02/repository/conf/user-mgt.xml.erb'),
        #require => File["/opt/wso2/wso2esb02"],
    }  -> 

    # Template for conf/carbon.xml
    file { '/opt/wso2/wso2esb02/repository/conf/carbon.xml':
        mode    => 644,
        owner   => "root",
        group   => "root",
        content => template('mw_install/wso2esb02/repository/conf/carbon.xml.erb'),
        #require => File["/opt/wso2/wso2esb02"],
    }  -> 

    # Template for conf/registry.xml
    file { '/opt/wso2/wso2esb02/repository/conf/registry.xml':
        mode    => 644,
        owner   => "root",
        group   => "root",
        content => template('mw_install/wso2esb02/repository/conf/registry.xml.erb'),
        #require => File["/opt/wso2/wso2esb02"],
    }  -> 

    # Template for conf/axis2/axis2.xml
    # TODO: Clustering should be configurable and enabled/disabled in this file
    file { '/opt/wso2/wso2esb02/repository/conf/axis2/axis2.xml':
        mode    => 644,
        owner   => "root",
        group   => "root",
        content => template('mw_install/wso2esb02/repository/conf/axis2/axis2.xml.erb'),
        #require => File["/opt/wso2/wso2esb02"],
    }  -> 

    # Template for conf/tomcat/catalina-server.xml'
    file { '/opt/wso2/wso2esb02/repository/conf/tomcat/catalina-server.xml':
        mode    => 644,
        owner   => "root",
        group   => "root",
        content => template('mw_install/wso2esb02/repository/conf/tomcat/catalina-server.xml.erb'),
        #require => File["/opt/wso2/wso2esb02"],
    }
